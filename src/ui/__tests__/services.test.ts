...
describe("Kusum E-Commerce Services", () => {
  ...
  describe("Product Filtering & Modest Search", () => {
    ...
    it("filters products by ready-to-wear category", () => {
      const pret = filterProducts({ category: "ready-to-wear" }, products);
      expect(pret.length).toBeGreaterThan(0);
      pret.forEach((p) => expect(p.category).toBe("ready-to-wear"));
    });
